...
- submit Lab 2 'Assignment' onto Canvas for marks
...
![](https://datasciencebook.ca/img/frontmatter/ds-a-first-intro-cover.jpg){width=50%}
...
a good practice is to check for NA and poke around the data first:
```{r data-smell-test, exercise=TRUE, exercise.lines = 4}
dim(pgs) # 344 penguins

pgs %>% 
  summarise_all(~ sum(is.na(.x))) # summarizes an important fact (e.g., the number of NA’s) per column
```

Side note: This is called purr-style lambda, starting with a tilde `~` and 
using `.x` in the function refers to every individual column that has been selected.

Decide how to deal with data anomalies and missing data:

```{r remove-na, exercise=TRUE, exercise.lines = 3}
pgs <- pgs %>% drop_na()

dim(pgs) # 333 penguins
```
...
After you create your account, **submit your Github handle on Canvas in Lab2**. 
It'll help us assign you to the repo of your project.
